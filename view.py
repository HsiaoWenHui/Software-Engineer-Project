...
class View2(ParentView):
    def __init__(self, master, model):
        ...
        self.playButton=tkinter.Button(master,image=self.playImage,text='PLAY',width=50,height=50,command=self.play,bg="white")
        self.inputText = tkinter.Label(master,image=self.logoImage,text="tetris",bg="black")    
        self.settingButton=tkinter.Button(master,image=self.setImage,text='SET',width=50,height=50,command=self.set,bg="white")
    ...
    def updateGame(self):
        #print("update")
        self.canvas.destroy()
        canvas=tkinter.Canvas(width=320,height=600,bg="#6B8E23")
       ...
       # print(self.mod.board_state)
        ...
        for i in range(0,15):
            for j in range(0,8):
                square=canvas.create_rectangle(j*40,i*40,j*40+40,i*40+40,fill="#BC8F8F")
                if self.board[i][j]==1:
                    square=canvas.create_rectangle(j*40,i*40,j*40+40,i*40+40,fill="#32CD32")#red
                elif self.board[i][j]==2:
                    square=canvas.create_rectangle(j*40,i*40,j*40+40,i*40+40,fill="	#FF7F50")#blue
                elif self.board[i][j]==3:
                    square=canvas.create_rectangle(j*40,i*40,j*40+40,i*40+40,fill="#6495ED")#green
                elif self.board[i][j]==4:
                    square=canvas.create_rectangle(j*40,i*40,j*40+40,i*40+40,fill="#778899")#yellow
                elif self.board[i][j]==5:
                    square=canvas.create_rectangle(j*40,i*40,j*40+40,i*40+40,fill="#B22226")#purple
                elif self.board[i][j]==6:
                    square=canvas.create_rectangle(j*40,i*40,j*40+40,i*40+40,fill="#004C99")#gray
                elif self.board[i][j]==7:
                    square=canvas.create_rectangle(j*40,i*40,j*40+40,i*40+40,fill="#FFFAFA")#orange
                else:
                    square=canvas.create_rectangle(j*40,i*40,j*40+40,i*40+40)
                #print("test"+str(self.board[i][j]))
